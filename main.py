import builtins, copy, json, keyword, os, random, shutil
...
class GUI(Tk):
    ...
    def __init__(self):
        ...
        if not os.path.exists('./questions/') or not os.listdir('./questions/'):
            self.import_questions()
        
        # Ensure an approprate file is being used to import
        while not self.get_dir_size():
            print("An Fatal Error Occured While Loading Questions!\nPerhaps '{self.config['import_path']}' is not the right file?")
    ...
    # Searches for session id or adds a placeholder to the searchbox if empty
    ...
    # Deletes current entry in searchbox
    ...
    def add_student(self):
        ...
        if self.question == '':
            self.print_debug('Please select a mastery (or enter id) for this student first.', colour=self.colours['error_fg'])
            return
    ...
    def centre(self, w, h, offsetx=0, offsety=0):
        return "{}x{}+{}+{}".format(w, h, (self.winfo_screenwidth()-w) // 2 - offsetx, (self.winfo_screenheight()-h) // 2 - offsety)
    ...
    # within the list of all files int he directory. Noteably, MT12, C2 originally had no V1, only V2
    def check_empty(self, g):
        for i in range(len(g)):
            for j in range(len(g[i])):
                for k in range(len(g[i][j])):
                    if len(g[i][j][k]) == 0:
                        self.print_debug(f'No question in input file at MT{i+1:02} Cat{j+1:02} Var{k+1:02}! Please Fix!', colour=self.colours['error_fg'])


    # Checks all newly imported questions to see if they are actually readabile.
    ...
    def check_readable(self, g):
        for q in os.listdir('./questions'):
            for c in os.listdir(f'./questions/{q}'):
                for v in os.listdir(f'./questions/{q}/{c}'):
                    with open(f'./questions/{q}/{c}/{v}', 'r', encoding='utf-8') as file:
                        try:
                            file.read()
                        except:
                            self.print_debug(f"Reading error in './questions/{q}/{c}/{v}'", colour=self.colours['error_fg'])
                            raise
    ...
    def create_widgets(self):
        ...
        self.num_progressions = len(os.listdir('./questions'))
        self.progress_buttons = []
        for i in range(self.num_progressions):
            self.progress_buttons += [Button(self, text=f'{i+1:02}', width=2, command=lambda i=i: self.get_questions(i))]
    ...
    def decode_id(self, s):
        ...
        self.print_debug(f'Decoded ID {str(data[0])[11:19]} MT{q_num+1:02} ' + ', '.join([f'Var {i[2]+1:02}' for i in data[1:]]))
        return data
    
    
    # Takes the current time and 1-3 tuples indicating the index of the MT, Category & Variation within the ./questions/ directory
    def encode_id(self, question_list):
        self.time = date.now()
        t = str(self.time)[11:19].split(':')
        session_id = hex(int(''.join([str(int(t[i]) + [18, 37, 29][i]) for i in range(3)] + [f"{i[2]:02}" for i in question_list] + [f"{question_list[0][0]:02}"])))[2:].upper()
        
        self.print_debug(f'Encoded ID {str(self.time)[11:19]} MT{question_list[0][0]+1:02} ' + ', '.join([f'Var {i[2]+1:02}' for i in question_list]))
        self.print_debug('Session ID: ' + session_id)
        
        return session_id
        
    
    # Looks for a needle in a haystack (or rather, the nth occurance of char needle)
    ...
    def get_dir_size(self):
        try:
            self.num_q = [[len(os.listdir(f'./questions/{i}/{j}')) for j in sorted(os.listdir(f'./questions/{i}'))] for i in sorted(os.listdir('./questions/'))]
            return True
        except:
            return False
    ...
    def gen_list(self):
        ...
        try:
            ...
            self.print_debug('This file does not match the designed import script! Is it the wrong file?', colour=self.colours['error_fg'])
    ...
    def get_questions(self, n):
        ...
        self.print_debug('Selected {}.'.format(', '.join([f'Var {self.question_list[i][2]+1:02}({self.num_q[n][i]:02})' for i in range(len(self.question_list))])))
            
        # Print out required files if necessary
        if require != []:
            self.print_debug('MT{:02} Requires Files: {}'.format(n+1, ', '.join(require)), colour=self.colours['require_fg'], always_print=True)
    ...
    # Wrapper function jsut to manage the import and checks from input.txt
    ...
    def open_student_window(self):
        if self.student_window == None:
            if not os.path.exists('./marks/') or len(os.listdir('./marks/')) == 0:
    ...
    def open_question(self, s=None):
        ...
        for i in range(len(result)):
            try:
                with open('./questions/Progression {:02}/Category {:02}/Variation {:02}.txt'.format(*result[i]), 'r', encoding='utf-8') as file:
                    self.question += f"Question {i+1}:\n{file.read()}\n\n\n"
                
            except FileNotFoundError:
                self.print_debug("File for MT{:02}, Cat {:02}, Var {:02} could not be found!".format(*result[i]), colour=self.colours['error_fg'])
                self.searchbox.config(bg=self.colours['bad'][1])
                return
    ...
    def update_widgets(self):
        ...
        # Tags
        self.question_field.tag_config("strings", foreground=self.colours['strings'][0], background=self.colours['strings'][1])
        self.question_field.tag_config("numbers", foreground=self.colours['numbers'][0], background=self.colours['numbers'][1])
        self.question_field.tag_config("keywords", foreground=self.colours['keywords'][0], background=self.colours['keywords'][1], font=(f2[0], f2[1], "bold"))
        self.question_field.tag_config("builtins", foreground=self.colours['builtins'][0], background=self.colours['builtins'][1])
...
class EntryPopup(Toplevel):
    ...
    def __init__(self, parent, prompt):
        ...
        self.s = StringVar()
        self.s.set(prompt)
        self.e = Entry(self, textvariable=self.s, font=self.parent.get_font(2), fg=self.parent.colours['placeholder_fg'], bg=self.parent.colours['textbox'][1])
        ...
        self.confirm.config(font=self.parent.get_font(1), fg=self.parent.colours['buttons'][0], bg=self.parent.colours['buttons'][1])
        self.confirm.config(activeforeground=self.parent.colours['active_button'][0], activebackground=self.parent.colours['active_button'][1])
        self.confirm.place(x=300, y=12.5, width=60, height=25)
        
    
    # Remove the placeholder text (although I think it focuses in  automatically now, so this probably happens immediately)
    def _focus_in(self, e):
        self.s.set("")
...
class SettingsWindow(Toplevel):
    ...
    def __init__(self, parent):
        ...
        self.tabs = (ttk.Frame(self.notebook, style="TFrame"), ttk.Frame(self.notebook, style="TFrame"), ttk.Frame(self.notebook, style="TFrame"))
    ...
    def draw(self):
        ...
        for key in self.cfg.keys():
            ...
            # Adds a trace, whih calls a function whenever the Var() is updated!
        ...
        self.labels = [
            ...
            (Label(self.tabs[1], text='Note: may need to restart program to update widget dimensions within the window.', wraplength=self.w-50), (25, 90, self.w-50, 40)),
        ...
        #Place the labels, whereever defined by the tuples above
        ...
        for key in self.cfg.keys():
            if type(self.cfg[key]) == bool:
                ...
                self.toggle_btns.append(Checkbutton(self.tabs[0], text=' '.join(key.split('_')).title(), variable=self.vars[key], anchor=W))
...
class StudentWindow(Toplevel):
    ...
    def add(self, name, progression, session_id, mark=None, start_time=None, elapsed_time=None):
        ...
        s['mark_btn'] = Button(self, textvariable=s['btn_text'], command=lambda s=s: self.next_mark(s))
        s['view_btn'] = Button(self, text='View', command=lambda s=s: self.parent.open_question(s['session_id']))
    ...
    # Autosave records every 15 mintues, without needing to close
    def autosave(self):
        n = date.now()
    ...
    def draw(self):
        ...
        self.geometry(self.parent.centre(self.w, self.h + 30*(len(self.students)-1), *self.parent.get_dimensions('win_stu')))
    ...
    # Reads all ./marks/*.json files and aggregates them into a single, human-readible .txt
    def export(self):
        if not os.path.exists('./marks/'):
            return
        
        data = []
        for filename in os.listdir('./marks/'):
    ...
    def next_mark(self, student):
        ...
        # confuse the marker or make them misclick.
    ...
    def read(self):
        ...
        for s in data:
            self.add(s['name'], s['mt_num'], s['session_id'], s['mark'], date.fromisoformat(s['start_time']), eval(s['elapsed_time']))
